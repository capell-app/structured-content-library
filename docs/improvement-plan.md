...
- **Auto-derive `published_at` when publishing without a date.** `scopePublished()` treats `published_at IS NULL` as visible, but the create/update actions set `published_at = null` whenever the picker is empty even for `Published` status. Net effect: published-with-no-date is visible (works) but the timestamp is lost for ordering/auditing. Default to `now()` on transition to `Published` when no date supplied. — correctness/UX — `src/Actions/CreateStructuredContentItemAction.php`, `src/Actions/UpdateStructuredContentItemAction.php` — **S**
...
- **No unique constraint backing import dedup; dedup skipped entirely when slug is null.** `existingItem()` returns `null` (→ always create) when slug is empty, so slug-less imports duplicate on every run. Combined with the missing unique index, this is a silent data-duplication risk. — `src/Actions/ImportStructuredContentItemsAction.php`, migration
